...
# Everything for the main program is stored in registers
...
power:
   ...
   movl 8(%ebp), %ebx        #put the first srgument in %eax
   movl 12(%ebp), %ecx       #put the second argument in %ecx
   movl %ebx, -4(%ebp)       #store the current result

power_loop_start:
   cmpl $1, %ecx             #if the power is one, we are done
   je   end_power
   cmpl $0, %ecx             #if power of 0
   je   zero  
   movl -4(%ebp), %eax       #store the current result
   imul %ebx, %eax           #multiple the current result by the base number
